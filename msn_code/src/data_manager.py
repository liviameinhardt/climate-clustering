...
class PatchesFolder(torch.utils.data.Dataset):

    def __init__(self, root, transform,size=None):
        self.root = root
        self.transform = transform
        
        self.imgs = np.load(root)
        if size is not None:
            idxs = np.random.choice(range(0,len(self.imgs)), size, replace=False)
    ...
    def __getitem__(self, index):
        ...
        tensor = torch.from_numpy(cur_img).unsqueeze(0).repeat(3, 1, 1)

        timg = self.transform(tensor)
